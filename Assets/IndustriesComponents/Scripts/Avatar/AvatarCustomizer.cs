...
namespace Fusion.Samples.IndustriesComponents
{
    public class AvatarCustomizer : MonoBehaviour
    {
        ...
        void DoConnect()
        {
            ...
            IAvatar currentAvatar = avatarRepresentation.currentAvatar;
            
            if (currentAvatar is SimpleAvatar simpleAvatar)
            {
                avatarUrlToSave = simpleAvatar.AvatarURL;
            }
            else if (currentAvatar is RPMAvatarLoader rpmAvatar)
            {
                avatarUrlToSave = rpmAvatar.AvatarURL;
            }
            
            if (!string.IsNullOrEmpty(avatarUrlToSave))
            {
                Debug.Log("Salvando a URL do avatar: " + avatarUrlToSave);
                PlayerPrefs.SetString(UserInfo.SETTINGS_AVATARURL, avatarUrlToSave);
                PlayerPrefs.Save();
            }
            else
            {
                Debug.LogWarning("Não foi possível obter a URL do avatar atual para salvar.");
            }
