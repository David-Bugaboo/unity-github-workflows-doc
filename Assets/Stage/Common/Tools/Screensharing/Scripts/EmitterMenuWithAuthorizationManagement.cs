...
/***
 * 
 * EmitterMenuWithAuthorizationManagement is in charge of displaying the screen sharing buttons 
 * and update text fields according to the user interaction with the menu or according to the ScreenShareStatus status.
 * 
 ***/

public class EmitterMenuWithAuthorizationManagement : MonoBehaviour
{
    ...
    void ToggleDesktopConnection(int desktopIndex)
    {
        ...
        {
            // the user click on the same button
            if (screenShareAttendee.ScreenShareStatus == ScreenShareStatus.ScreenShareRequested)
            {
                // Cancel the request
                screenShareAttendee.ScreenShareStatus = ScreenShareStatus.ScreenShareCanceled;
            }
            ...
            else
            {
                ...
                screenShareAttendee.ScreenShareStatus = ScreenShareStatus.ScreenShareRequested;
            }
        }
        else
        {
            // the user click on the other button
            if (screenShareAttendee.ScreenShareStatus == ScreenShareStatus.ScreenShareRequested)
            { // do nothing because we keep the same request for both desktop
            }
            else if (screenShareAttendee.ScreenShareStatus == ScreenShareStatus.ScreenShareInProgress)
            {
                // change the uWindowsCaptureRecorder Desktop Index
                OnSelectDesktop(desktopIndex);
            }
            else
            {
