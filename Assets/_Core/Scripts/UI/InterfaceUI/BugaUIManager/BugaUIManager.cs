...
public class BugaUIManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Define a primeira sess�o de telas como a sess�o atual
    ...
    public void ShowScreen(string screenName)
    {
        foreach (GameObject screen in currentScreenSessionScreens)
        {
            ...
            screen.GetComponent<BugaScreen>().ToShow(show);
    ...
    {
        if (sessionName.Equals("LoginCPF") && !string.IsNullOrEmpty(UserManager.Instance.CurrentUser.id) && !UserManager.Instance.CurrentUser.onboarded)
        {
            var index = screenSessions.FindIndex(s => s.name.Equals(sessionName));
            SetCurrentScreenSession(index + 1);
            return;
        }
    ...
    {
        // Desativa todas as telas da sess�o atual
        foreach (GameObject screen in currentScreenSessionScreens)
        {
        ...
        foreach (GameObject screen in screensToShow)
        {
            ...
            screen.GetComponent<BugaScreen>().ToShow(true);
        ...
        {
            //screen.SetActive(false);
            screen.GetComponent<BugaScreen>().ToShow(false);
            ScreenToShowChanged?.Invoke(screen.name, false);

            Debug.Log($"Screen hidden: {screen.name}");
        }

        // Atualiza a lista de telas da sess�o atual
...
public class UIScreenSession
{
    ...
    public bool isCurrentSession; // Propriedade para indicar se � a sess�o atual
}